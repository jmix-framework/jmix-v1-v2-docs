...
[[lock]]
=== Блокировка
Метод `lockMode()` с параметром из перечисления `jakarta.persistence.LockModeType` используется для задания блокировки JPA сущностей на уровне базы данных. В следующем примере показано получение пессимистичной блокировки при выполнении SQL запроса вида `select ... for update`:
