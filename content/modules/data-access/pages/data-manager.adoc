...
如果不需要返回保存的实例，可以使用 `saveWithoutReload()` 方法。这样 `DataManager` 便不会从数据库读取保存后的实体，从而能提高性能。示例：
...
如果需要保存实体集合（`List`，`Set` 等），并获取保存后的实例，那么可以用 `saveAll()` 方法。示例：
...
使用 `saveWithoutReload()` 也可以保存实体集合，如果不需要获取保存后的实体实例，则推荐使用该方法以提高性能。
...
首先，不要一个一个实体调用单独的 `save(entity)` 方法，而是可以将所有实体（数量小于 1000）的集合放在一个事务中保存。

如果不需要返回保存后的实例，可以使用 `saveWithoutReload()` 方法。在 `SaveContext` 对象中，可以通过 `setDiscardSaved(true)` 实现相同的功能。示例：
