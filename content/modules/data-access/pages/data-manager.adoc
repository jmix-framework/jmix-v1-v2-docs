...
If the entity with the given identifier may not exist, instead of `one()` terminal method use `optional()` which returns `Optional<E>`. In the example below, if the entity is not found, a new instance is created and returned:
...
The `loadValues(String query)` method loads a list of `KeyValueEntity` instances populated with the given query result. For example:

[source,java,indent=0]
----
include::example$/data-access-ex1/src/main/java/com/company/demo/bean/OrderService.java[tags=load-values]
----
<1> Specify the data store where the requested entities are located. Omit this method if the entity is located in the main data store.
...
<2> Specify the data store where the requested entities are located. Omit this method if the entity is located in the main data store.

[[load-scalars-limitations]]
==== Limitations

The `loadValues()` and `loadValue()` have the following limitations:

. The query must be a valid JPQL query for the single specified data store. It can contain only persistent attributes of JPA entities.

. xref:data-model:entities.adoc#cross-data-store-ref[Cross-datastore references] are not supported.

. These methods are not implemented in xref:rest-ds:index.adoc[].
...
If you don't need the saved instance immediately, use the `saveWithoutReload()` method. This will improve performance because `DataManager` will not fetch the saved entity back from the database. For example:

[source,java,indent=0]
----
include::example$/data-access-ex1/src/main/java/com/company/demo/bean/CustomerService.java[tags=save-without-reload]
----
...
If you need to save a collection of entities (`List`, `Set`, etc.), and return the saved instances, use the `saveAll()` method. For example:

[source,java,indent=0]
----
include::example$/data-access-ex1/src/main/java/com/company/demo/bean/CustomerService.java[tags=save-all]
----

The `saveWithoutReload()` method also accepts multiple instances and collections. Use it instead of `save()` and `saveAll()` if you don't need the saved entities for further actions.

The most powerful form of the `save()` method accepts a `SaveContext` object that can be used to add multiple instances and specify additional parameters of saving. In the example below, we save a collection of entities using `SaveContext`:
...
First of all, instead of passing each entity instance to separate calls of the `save(entity)` method, consider saving all entities (if the collection is not very large, say, less than 1000) in a single transaction by invoking the save method once for the whole collection.

If you don't need the saved instances immediately, use `saveWithoutReload()` method. To achieve the same result when using the `SaveContext` object, invoke the `setDiscardSaved(true)` method on it. For example:
...
When saving entities, use the `setJoinTransaction(false)` method of `SaveContext` to begin and commit a separate transaction for the operation with this context:
