...
Значение этого параметра определяется локалью текущей пользовательской сессии, которая извлекается из объекта xref:security:authentication.adoc#current[CurrentAuthentication].
