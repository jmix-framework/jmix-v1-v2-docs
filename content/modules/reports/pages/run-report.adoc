...
The easiest way to run reports is from the xref:reports-ui.adoc#run-report-view[Run report] view. The user must have the permission to access this view. The list will contain all reports that are xref:creation/permissions.adoc[available to the user] in accordance with their role. If the report has external parameters, they will be requested in a special form when running the report.

[[run-actions]]
== Running from Views

Users can run reports from arbitrary views using special actions and associated buttons or component context menu items. In this case, the xref:creation/permissions.adoc[availability] of the report in this view is checked in addition to the user role.
...
`io.jmix.reportsflowui.action.RunReportAction` is a xref:flow-ui:actions/standard-actions.adoc[standard action] for running the reports associated with the current view or list component. It should be defined for a `Button` or a list component (xref:flow-ui:vc/components/dataGrid.adoc[], xref:flow-ui:vc/components/treeDataGrid.adoc[]).
...
<2> The `type` attribute defines a specific `report_runReport` action type, provided by the Reports add-on.
...
`io.jmix.reportsflowui.action.RunListEntityReportAction` is a xref:flow-ui:actions/standard-actions.adoc[standard action] for printing reports for entity instances associated with a list component (xref:flow-ui:vc/components/dataGrid.adoc[], xref:flow-ui:vc/components/treeDataGrid.adoc[]).
...
TIP: Use xref:jmix:studio:code-snippets.adoc[] to generate code for running reports using the API.
