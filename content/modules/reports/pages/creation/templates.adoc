...
You can add report templates in the report detail view. Create or edit an existing report in the reports list view and switch to the *Templates* tab:
...
* `rootBand` - an object of the `BandData` type that provide access to band datasets.
...
When a report is executed, cell whose content exceeds the available space for a single line will automatically become multi-line, with the row height adjusting accordingly to display the entire contents.
====
