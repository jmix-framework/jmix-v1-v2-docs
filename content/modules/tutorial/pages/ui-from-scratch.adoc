...
右键点击 *Jmix UI* 结构面板中的 `layout` 元素，选择 *Add Component* 菜单。找到并双击 `DataGrid` 组件。在 *DataGrid* 弹窗中，选择 `userStepsDc` 数据容器，并反选所有的 *Actions*：
...
在 fetch plan 和列中添加 `step.name` 属性，并删除不需要的 `sortValue` 列。
