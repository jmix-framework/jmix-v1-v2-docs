...
在列表或详情视图时，可以通过 xref:flow-ui:actions/standard-actions.adoc[标准操作] 从主视图打开其他视图，也可以编程式打开。
...
通常我们需要编辑显示在列表数据视图组件中的一个实体，例如，xref:flow-ui:vc/components/dataGrid.adoc[dataGrid]。那么我们需要按照下面的方式调用，这样更方便并能自动更新 `dataGrid`：
...
打开查找视图也类似，与详情视图一样，多数情况是通过标准操作打开，例如，xref:flow-ui:actions/entity-picker-actions.adoc#entity_lookup[entity_lookup]。下面的示例演示使用 `ViewBuilders` API，在不使用标准操作时也很方便。
