...
用在 Spring Security {spring-security-api}/org/springframework/security/authentication/AnonymousAuthenticationProvider.html[AnonymousAuthenticationProvider^] 和
{spring-security-api}/org/springframework/security/web/authentication/AnonymousAuthenticationFilter.html[AnonymousAuthenticationFilter^] 中的 key。
...
每个用户可以拥有的最大 xref:security:authentication.adoc#user-session[session] 数量。

这个属性会影响 Spring Security 的 {spring-security-api}/org/springframework/security/config/annotation/web/configurers/SessionManagementConfigurer.html#maximumSessions(int)[SessionManagementConfigurer.maximumSessions^] 参数。
