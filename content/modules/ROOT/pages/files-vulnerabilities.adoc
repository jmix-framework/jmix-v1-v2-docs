...
+
Параметр запроса может быть изменен для возврата файла с типом `Content-Type` равным `text/html` если `name` оканчивается на `.html`. Это может привести к выполнению вредоносного JavaScript в браузере при загрузке файла, подготовленного злоумышленником в хранилище файлов.

. *DoS в локальном хранилище файлов*
+
Реализация локального хранилища файлов не ограничивает размер загружаемых файлов. Злоумышленник может воспользоваться этим, загружая файлы чрезмерно большого размера, что может привести к исчерпанию свободного места на сервере и возвращению ошибки 500, вызывая отказ в обслуживании.
...
* Jmix 2.0.0 - 2.3.4
* CUBA 6.2.0 - 7.2.22
* CUBA REST API дополнение 7.1.1 - 7.2.6
* CUBA JPA Web API дополнение 1.0.0 - 1.1.0
