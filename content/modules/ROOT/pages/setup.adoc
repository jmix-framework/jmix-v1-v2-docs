...
+
If you have any project open, use the main menu: *File* -> *Settings* (on Windows and Linux) or *IntelliJ IDEA* -> *Preferences* (on macOS). Then switch to the *Plugins* section.
...
NOTE: To access premium Studio features, configure a subscription in the xref:studio:subscription.adoc[subscription] dialog window.
...
To enable an AI agent to work effectively with Jmix patterns, architecture, and best practices, we recommend using the https://github.com/jmix-framework/jmix-agent-toolkit/tree/v2[Jmix Agent Toolkit^]. This toolkit provides:
...
* Configuration of recommended MCP servers and CLI tools.
