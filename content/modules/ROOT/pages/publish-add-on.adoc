...
如需关联至特定的 Jmix 版本，请在 https://github.com/jmix-framework/jmix/issues[^] 创建一个 issue，并提供兼容的扩展组件和 Jmix 版本信息。

或者，也可以在相应的 https://github.com/jmix-framework/jmix/branches/all?query=release_[版本分支^] 提交 PR。

1. Fork https://github.com/jmix-framework/jmix[jmix-framework/jmix^] 仓库。

2. 切换至你想要添加版本的 release 分支（例如，`{page-origin-branch}`）。

3. 在 `jmix-bom/bom.gradle` 中的 `community add-ons` 备注后添加带有 Maven 坐标和正确版本的内容：
+
[source,gradle]
.bom.gradle
----
...
4. 创建目标分支的 PR：`jmix-framework/jmix` 仓库的 `{page-origin-branch}` 分支。

[NOTE]
====
当发布新的主要或次要 Jmix 版本（例如，5.5.0）时，其 BOM 不包含任何第三方插件，因为没有经过与新版本的兼容性测试。

你需要为相应的 release 分支（例如，`release_5_5`）提交新的 PR，将扩展组件包含到新的 BOM 中。
在组件添加到 BOM 之前，如果用户在其构建脚本中明确指定组件版本，仍然可以在新的 Jmix 版本中使用扩展组件。
====
