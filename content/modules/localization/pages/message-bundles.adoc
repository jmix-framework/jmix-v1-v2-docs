...
The `MessageTools` bean provides utility methods for obtaining localized names of data model elements in Java code. It is useful when you need the same entity and attribute captions that Jmix displays in UI components.

The `getEntityCaption()` method returns the localized name of an entity. The `getPropertyCaption()` method returns the localized name of an entity attribute:
...
These methods use the data model localization keys described in the <<localizing-data-model,Localizing Data Model>> section. In the example above, `getEntityCaption()` reads the `com.company.demo.entity/User` key, and `getPropertyCaption()` reads the `com.company.demo.entity/User.username` key.
