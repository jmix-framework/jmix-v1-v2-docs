...
参考 xref:restds-oidc-guide:index.adoc[] 了解实战案例。
...
[NOTE]
====
在 Kotlin 项目中，还需要对 `User` 实体类做如下修改：

* 为 `email` 属性添加 `@get:JvmName("getEmail_")` 注解。
* 添加重写方法 `override fun getEmail(): String? = email`
====
...
假设要保护所有以 `/authenticated/` 开头的 URL，而所有以 `/anonymous/` 开头的 URL 都可供匿名访问。有 xref:security:custom-endpoints.adoc#token-based-authentication[几种实现的方式]，其中最简单的就是使用应用程序属性配置：
