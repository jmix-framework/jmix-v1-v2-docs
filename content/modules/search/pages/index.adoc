...
Java API clients for Elasticsearch/OpenSearch are forward-compatible with server versions.

The version of the Elasticsearch Java API used by the add-on is determined by the Spring Boot. See `co.elastic.clients:elasticsearch-java` version at {spring-boot-doc}/appendix/dependency-versions/coordinates.html[^].

With OpenSearch, the add-on uses Java API client v2.13.
