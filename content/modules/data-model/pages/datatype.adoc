...
[NOTE]
====
Реализации `Datatype` являются бинами Spring (потому что `@DatatypeDef` сам аннотирован `@Component`), но другие бины, например xref:localization:message-bundles.adoc#using-messages-in-java[Messages], нельзя напрямую инжектировать в классы datatype используя `@Autowired`, так как экземпляры datatype инициализируются в самом начале процесса запуска приложения и такая инжекция может вызвать циклическую зависимость.
...
<4> Аннотация `@Ddl` указывает, что Studio должна генерировать для атрибутов сущности в базе данных колонку типа `varchar(255)`. Подробнее см. <<ddl-annotation,ниже>>.
...
Аннотация `@Ddl` на классе datatype позволяет указать, какой SQL-тип должен использоваться для атрибутов сущности. Studio учитывает эту аннотацию при генерации xref:db-migration.adoc[скриптов миграции базы данных].

SQL-тип указывается в атрибуте `value` аннотации, например:
...
Если необходимо указать разные SQL-типы для разных СУБД, добавьте несколько аннотаций `@Ddl` с атрибутами `dbms`. В следующем примере столбец базы данных будет иметь тип `text` в PostgreSQL, тип `varchar2(255)` в Oracle и тип `varchar(255)` во всех остальных СУБД:
...
Studio распознает следующие значения в атрибуте `dbms`:
