...
The task renderer is defined by a callback function - `taskRendererFunction`.

The `taskRendererFunction` allows you to customize how individual tasks are rendered in the Kanban board.

The callback function receives two parameters:

* `task` (HTMLElement). The HTML element representing the task card that you can modify.
* `data` (Object). The task data object containing all task properties.
...
For complete API details, see https://www.htmlelements.com/docs/kanban-api/#toc-ontaskrender_any[Kanban JavaDoc^].

[[columnHeaderRendererFunction]]
=== Header Renderer

The header renderer is defined by a callback function - `columnHeaderRendererFunction`.

The `columnHeaderRendererFunction` allows you to customize how Kanban column headers are rendered.

The callback function receives three parameters:

* `header` (HTMLElement). The HTML element representing the column header that you can modify.
* `data` (Object). The column data object containing column properties.
* `dataField` (String). The data field name associated with this column (matches the `dataField` attribute in the column definition).
...
For complete API details, see https://www.htmlelements.com/docs/kanban-api/#toc-oncolumnheaderrender_any[Kanban JavaDoc^].

[[columnFooterRendererFunction]]
=== Footer Renderer

The footer renderer is defined by a callback function - `columnFooterRendererFunction`.

The `columnFooterRendererFunction` allows you to customize how Kanban column footers are rendered. Column footers are typically used to display summaries, statistics, or action buttons for each column.

To use column footer rendering, ensure these attributes are enabled:

[source,xml]
----
<kanban:kanban columnSummaryEnabled="true"
               columnFooterVisible="true">
----

The callback function receives three parameters:

* `footer` (HTMLElement). The HTML element representing the column footer that you can modify.
* `data` (Object). The column data object containing column properties and summary information.
* `dataField` (String). The data field name associated with this column (matches the `dataField` attribute).
...
For complete API details, see https://www.htmlelements.com/docs/kanban-api/#toc-oncolumnfooterrender_any[Kanban JavaDoc^].
...
The following attributes are specific to `kanban`:
...
|`false`
|[[taskDueDateVisible]]taskDueDateVisible
|If `true`, displays the task due icon. See <<propertyDueDate, dueDate>>.
|`false`
|[[taskPosition]]taskPosition
...
The following handlers are specific to `kanban`:
