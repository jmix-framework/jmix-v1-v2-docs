...
A business key is a business-specific identifier of a process instance. It associates the instance with the relevant business entity or context, providing a more descriptive alternative to the system-generated process instance ID.
...
Adding a business key helps users identify which context the process belongs to. It can be added in three ways:

. *Input dialog form* for the start event. See the xref:bpm:input-dialog.adoc#business-key[example].
. *Jmix view form* using `ProcessFormContext`. See the xref:bpm:bpmn/jmix-view-forms.adoc#business-key[example].
. Programmatically using API. See the xref:bpm:jmix-bpm-api.adoc#business-key[example].
