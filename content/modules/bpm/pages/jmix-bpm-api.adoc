...
Jmix BPM uses the <<flowable-api, Flowable API>>, which facilitates easy interaction between the Spring Boot application and the process engine. The <<flowable-rest-api,REST API>> of Flowable offers extensive integration capabilities with external applications.

[[flowable-api]]
== Flowable API

Flowable API allows programmatic interaction with the process engine. This API allows to start process instances, complete tasks, and execute various queries, for example, get a list of tasks for a user or get active instances of a process definition.
...
See https://www.flowable.com/open-source/docs/bpmn/ch14-REST
...
See https://www.flowable.com/open-source/docs/dmn/ch07-REST
