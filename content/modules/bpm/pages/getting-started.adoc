...
本节介绍如何建模和运行一个简单的业务流程。为此，我们将使用 BPM 扩展组件运行时工具创建一个用于申请和批准报销的流程。

TIP: 也可以用 Jmix Studio 内置的工具创建这个流程。请参见 xref:bpm:creating-process-in-studio.adoc[] 中的详细说明。
...
画布中的每个元素都有其自有属性。此时，画布中唯一的元素只有 `StartEvent`，表示发起流程的用户所能看到的 <<start-form, 启动流程表单>>。下面我们开始配置：
...
+
*  设置 *Business key* 字段的值为 `requestNumber`。这是一个流程实例在特定领域的标识符。
...
创建一个用于审批的任务。审批任务将用 <<approval-form,审批表单>> 表示，只有被指定可以审批该笔报销的经理才能看到。
...
* 点击 image:quick-start/plus-button.png[[align="center", width="36"] a添加下列已有参数，并设置为不可编辑：
+
[cols="1,1,1,1,1"]
|===
|Process variable | Caption | Type | Editable | Required
...
* 画布中选中审批（Approval）元素。然后在 *Outcomes* 属性中点击 image:quick-start/plus-button.png[align="center", width="36"]。
* 为第一个输出设置下列属性：
// +
// [cols="1,1,1"]
// |===
...
现在流程已经完成了建模并部署，下一步可以启动了。我们一步一步进行，同时查看每一步中我们创建的表单。
