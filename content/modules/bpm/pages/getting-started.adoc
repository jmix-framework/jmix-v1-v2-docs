...
Этот раздел содержит рекомендации для быстрого старта. Здесь описаны шаги по созданию и запуску простого бизнес-процесса с помощью инструментов BPM, доступных в веб-приложении.

TIP: Вы также можете создать этот процесс, используя инструменты в Jmix Studio. Для этого следуйте инструкциям в статье xref:bpm:creating-process-in-studio.adoc[]

Мы создадим приложение в котором реализован процесс возмещения расходов.
...
Запустите приложение и перейдите на экран *BPM -> Modeler*. На панели свойств справа отображаются свойства текущего процесса.
