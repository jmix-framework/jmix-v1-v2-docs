...
A *Service Task* is used to invoke Java code, external services, APIs, or automated applications, to complete its function.
...
Parameter *Task type* defines the way service task is implemented.
...
If you select _JavaDelegate class_ option in the _Task type_ combo box, you can select an appropriate class implementing `JavaDelegate`:
...
To use `JavaDelegate` classes in delegate expressions, annotate them with @Component, similar to a Spring bean.
