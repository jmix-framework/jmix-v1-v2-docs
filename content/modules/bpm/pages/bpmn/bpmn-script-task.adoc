...
* *scriptFormat*: an extended attribute that specifies the script language; currently, only Groovy is supported. See https://groovy-lang.org/documentation.html[Groovy language documentation^].

* *script*: the script to execute. Use the *Script editor* to edit it.

* *resultVariable*: This optional attribute saves the evaluation result in a variable with the specified name within the execution context. See <<script-result, details>>.

image::bpmn-script-task/script-task-properties.png[,473]

WARNING: The script body cannot be empty, otherwise it causes a parsing error during deployment.
