...
Beans may be accessed in scripts by their names. The following Groovy script calls the `doSomething` method from `MyBean` and sets the result in the execution context as a variable:
...
TIP: The primary and recommended way to call a bean is with a xref:bpmn/bpmn-service-task.adoc[]. If you need to use Groovy logic along with Java bean calls, call the bean from a Service Task before or after the Script Task and let them communicate via process variables.
