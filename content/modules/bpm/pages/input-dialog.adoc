...
Studio modeler lets the business key be set directly or sourced from a process variable.

NOTE: xref:jmix:bpm:menu-views/modeler-web.adoc[Web Modeler] supports four sources for the business key: a process variable, an Entity attribute (e.g., `${entity.attribute}`), a Spring bean method call, or Flowable https://www.flowable.com/open-source/docs/bpmn/ch04-API/#expressions[expression^].
