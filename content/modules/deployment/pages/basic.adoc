...
By default, the image will be created with the name of your project and a tag that is equal to your project's version.
...
Deploying more than one Jmix WAR on the same Tomcat server can cause a JMX beans names clash, producing:

----
Caused by: javax.management.InstanceAlreadyExistsException: jmix.ui:type=TemporaryStorage
----

To resolve this issue give each WAR its own JMX domain. Add this to the application properties:
