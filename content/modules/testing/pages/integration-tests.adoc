...
Jmix allows code to be executed on behalf of a specified user, which is often necessary for testing functionality that relies on user roles and permissions. This can be achieved by using the xref:security:authentication.adoc#system[SystemAuthenticator].
...
As the user has no roles, the `CustomerService` returns an empty `Optional`.
...
Sometimes, even in integration tests, it is necessary to mock out certain parts of the application. In those cases, it is possible to combine the functionalities of `@SpringBootTest` with Mockito and mock out particular beans, but still use the overall Spring context.
...
. To simulate a failure scenario in which email sending did not work.
...
<4> The behavior of the mock is specified and the configured mock is returned.
