...
You can specify a set of actions in an XML view descriptor for any component that implements the `HasActions` interface (xref:vc/components/dataGrid.adoc[], xref:vc/components/entityComboBox.adoc[], xref:vc/components/entityPicker.adoc[], and others), or in the view itself. You can do it in the `actions` element, which contains nested `action` elements.
...
* `shortcutCombination` - a xref:keyboard-shortcuts.adoc[keyboard shortcut] to execute the action. For example:
+
[source,xml,indent=0]
----
