...
NOTE: 默认情况下，`ExceptionDialog` 显示为一个模态窗。该行为可以通过 xref:flow-ui:ui-properties.adoc#jmix.ui.exception-dialog-modal[jmix.ui.exception-dialog-modal] 应用程序属性在 `application.properties` 文件中配置。
