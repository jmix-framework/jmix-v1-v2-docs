...
Диалог, отображаемый обработчиком исключений по умолчанию, можно настроить с помощью специальных CSS-переменных и свойств приложения, или путем расширения Java-класса диалога и реализации собственной логики.

[[using-css-and-properties]]
=== Использование CSS и свойств

Размеры диалога обработчика исключений по умолчанию можно настроить с помощью CSS-переменных в вашем xref:flow-ui:themes/themes.adoc#application-theme[CSS-файле темы приложения]. Например:

[source,css]
----
include::example$/onboarding/frontend/themes/onboarding/onboarding.css[tags=html;exception-dialog]
----

По умолчанию `ExceptionDialog` отображается как модальное окно. Это поведение можно настроить, установив свойство xref:flow-ui:ui-properties.adoc#jmix.ui.exception-dialog-modal[jmix.ui.exception-dialog-modal] в файле `application.properties`.

[[extending-exception-dialog-class]]
=== Расширение класса ExceptionDialog

В примере ниже показано, как добавить кнопку, позволяющую пользователям сообщать об исключении системному администратору.
...
== Обработка исключений навигации

При возникновении необработанного исключения во время навигации Vaadin отображает специальный экран ошибки. Например, когда пользователь переходит по несуществующему маршруту, отображается следующая страница (в продакшн-сборке доступные маршруты не показываются):

.Пример страницы по умолчанию для ошибки не найденного маршрута
image::exception-handlers/original-error-page.png[align="center", width="821"]

По умолчанию реализованы классы `RouteNotFoundError` для `com.vaadin.flow.router.NotFoundException` и `RouteAccessDeniedError` для `com.vaadin.flow.router.AccessDeniedException`.

Чтобы предоставить собственную страницу ошибки, создайте класс, расширяющий стандартный, например:
...
.Пример пользовательской страницы для ошибки не найденного маршрута
image::exception-handlers/custom-error-page.png[align="center", width="821"]

Ваш класс будет автоматически обнаружен фреймворком и использован для отображения ошибок навигации.

См. также {vaadin-docs}/flow/routing/exceptions[документацию Vaadin^] по обработке исключений навигации.