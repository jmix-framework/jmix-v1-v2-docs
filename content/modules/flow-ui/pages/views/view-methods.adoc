...
* Two callbacks allow you to customize the header and footer of views displayed in a dialog window. To utilize these callbacks, you must override them within your view class.

** `configureDialogWindowHeader()` - this callback configures the xref:flow-ui:views/opening-views.adoc#dialog-windows[DialogWindow] header when the view is opened in `OpenMode.DIALOG`. It provides access to the header component, enabling you to add custom components, titles, or styling elements.
+
[source,java,indent=0]
----
include::example$/onboarding/src/main/java/com/company/onboarding/view/department/DepartmentListView.java[tags=dialog-header]
----

** `configureDialogWindowFooter()` - this callback configures the xref:flow-ui:views/opening-views.adoc#dialog-windows[DialogWindow] footer when the view is opened in `OpenMode.DIALOG`. It provides access to the footer component, enabling you to add custom buttons, actions, or informational elements.
+
[source,java,indent=0]
----
include::example$/onboarding/src/main/java/com/company/onboarding/view/department/DepartmentListView.java[tags=dialog-footer]
----
+
The screenshot below illustrates an example of a dialog window with customized header and footer.
+
image::views/dialog-header-footer.png[align="center",width="802"]
...
* `getSaveNotificationText()` - returns the save notification text. The default implementation generates the text based on the entity and instance names. You can override this method to return your own text.
...
* `getLockStatus()` - returns the xref:pessimistic-lock:index.adoc[pessimistic lock] status of the currently edited entity instance. Possible variants:
