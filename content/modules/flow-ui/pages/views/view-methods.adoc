...
** `configureDialogWindowHeader()` - 这个回调方法可以在视图以 `OpenMode.DIALOG` 模式打开时配置 xref:flow-ui:views/opening-views.adoc#dialog-windows[DialogWindow] 的 header。该方法提供了对 header 组件的访问，可以添加自定义的组件、文字或样式元素。
+
[source,java,indent=0]
----
...
** `configureDialogWindowFooter()` - 这个回调方法可以在视图以 `OpenMode.DIALOG` 模式打开时配置 xref:flow-ui:views/opening-views.adoc#dialog-windows[DialogWindow] 的 footer。该方法提供了对 footer 组件的访问，可以添加自定义的组件、文字或样式元素。
+
[source,java,indent=0]
----
...
----
+
下面的截图是上面自定义 header 和 footer 代码的运行结果：
+
