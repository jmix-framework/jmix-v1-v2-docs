...
----
include::example$/onboarding/src/main/java/com/company/onboarding/view/user/UserDetailEventsView.java[tags=timeZoneField;init-event]
----
...
`ValidationEvent` отсылается в экранах, унаследованных от `StandardDetailView`, когда экран проходит валидацию при сохранении данных. Используйте обработчик этого события для выполнения дополнительной проверки экрана.

Для возврата ошибок валидации и прерывания процесса сохранения используйте объект `ValidationErrors`, возвращаемый методом `getErrors()` события. Например:
...
Если объект `ValidationErrors` не пуст после окончания процесса валидации, операция сохранения не будет выполнена, и экран отобразит ошибки, используя метод `ViewValidation.showValidationErrors()`.

См. xref:views/view-validation.adoc[] для дополнительной информации.
...
[[read-only-change-event]]
== ReadOnlyChangeEvent

Событие `ReadOnlyChangeEvent` генерируется экранами, унаследованными от `StandardDetailView`, при изменении их состояния "только для чтения". Обычно это происходит при переключении пользователя между режимом просмотра и режимом редактирования.

Подпишитесь на это событие, чтобы синхронизировать состояние "только для чтения" пользовательских компонентов с общим режимом экрана. Это обеспечивает согласованное поведение элементов интерфейса при взаимодействии с пользователем.

[source,java,indent=0]
----
include::example$/onboarding/src/main/java/com/company/onboarding/view/user/UserDetailEventsView.java[tags=timeZoneField;read-only-change-event]
----
