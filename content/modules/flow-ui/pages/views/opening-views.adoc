...
There are several ways to set the size of a dialog window:

. *Using the `@DialogMode` annotation*
+
Add the `@DialogMode` annotation to the view class that is opened as a dialog. This sets the default size for the dialog when the view is opened.
+
[source,java,indent=0]
----
