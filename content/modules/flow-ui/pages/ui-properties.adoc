...
这个应用程序属性控制是否在 Jmix 应用程序中使用模态窗显示 `ExceptionDialog`。

如果启用，则 `ExceptionDialog` 作为模态窗显示，用户只有先关闭该窗口才能与应用程序的其他部分交互。

如果禁用，则 `ExceptionDialog` 不作为模态窗显示，用户可以与应用程序的其他部分交互而无需关闭该窗口。

默认值：`true`。

参阅 xref:flow-ui:exception-handlers.adoc[异常处理]。
...
定义关闭当前视图使用的 xref:keyboard-shortcuts.adoc[键盘快捷键]。

[[jmix.ui.view.save-shortcut]]
=== jmix.ui.view.save-shortcut

定义保存数据更改的 xref:keyboard-shortcuts.adoc[键盘快捷键]。
...
启用在 Masquerade 测试库中生成特殊测试 ID 的功能。具体请参阅 xref:testing:masquerade.adoc[Masquerade]。
...
该属性控制当 xref:vc/components/checkbox.adoc[checkbox] 和 xref:vc/components/switch.adoc[switch] 绑定一个带 `@NotNull` 验证的必需布尔属性时的行为。

* 如果配置为 `true`，则 `checkbox`/`switch` 必需勾选以通过验证。

* 如果配置为 `false`，则 `checkbox`/`switch` 可以保留非选择状态而不会导致验证错误。这样可以表示所需的 `true` 或 `false` 值。
...
定义特定实体选择组件中需要添加的操作，该组件用在 xref:vc/components/genericFilter.adoc[genericFilter] 的参数中，或可编辑的 xref:vc/components/dataGrid.adoc[dataGrid] 单元格中。

例如，如果需要在选择 `User` 实体的组件中使用 `entity_lookup`、`entity_open` 和 `entity_clear` 操作，可以按如下设置：
...
定义选择特定实体时使用的选择组件，该组件用在 xref:vc/components/genericFilter.adoc[genericFilter] 的参数中，或可编辑的 xref:vc/components/dataGrid.adoc[dataGrid] 单元格中。

属性值需要设置为类的全限定名称（FQN）。例如，如果需要为 `User` 实体使用 xref:vc/components/entityComboBox.adoc[entityComboBox] 组件，可以按如下设置：
...
默认情况下，通用机制使用 xref:vc/components/entityPicker.adoc[entityPicker] 组件。
...
设置 xref:vc/components/genericFilter.adoc[genericFilter] 组件应用当前过滤条件的 xref:keyboard-shortcuts.adoc[快捷键]，可用在过滤器不是 <<jmix.ui.component.filter-auto-apply, 自动应用>> 时。
...
定义执行 xref:actions/list-actions.adoc#list_add[list_add] 操作的 xref:keyboard-shortcuts.adoc[键盘快捷键]。

[[jmix.ui.component.grid-create-shortcut]]
=== jmix.ui.component.grid-create-shortcut

定义执行 xref:actions/list-actions.adoc#list_create[list_create] 操作的 xref:keyboard-shortcuts.adoc[键盘快捷键]。

[[jmix.ui.component.grid-edit-shortcut]]
=== jmix.ui.component.grid-edit-shortcut

定义执行 xref:actions/list-actions.adoc#list_edit[list_edit] 操作的 xref:keyboard-shortcuts.adoc[键盘快捷键]。
...
定义执行 xref:actions/list-actions.adoc#list_read[list_read] 操作的 xref:keyboard-shortcuts.adoc[键盘快捷键]。
...
定义执行 xref:actions/list-actions.adoc#list_remove[list_remove] 操作的 xref:keyboard-shortcuts.adoc[键盘快捷键]。
...
定义清空选择器组件输入内容的 xref:keyboard-shortcuts.adoc[键盘快捷键]。

[[jmix.ui.component.picker-lookup-shortcut]]
=== jmix.ui.component.picker-lookup-shortcut

定义选择器组件中打开选择视图的 xref:keyboard-shortcuts.adoc[键盘快捷键]。

[[jmix.ui.component.picker-open-shortcut]]
=== jmix.ui.component.picker-open-shortcut

定义选择器组件中打开已选实体详情视图的 xref:keyboard-shortcuts.adoc[键盘快捷键]。

[[jmix.ui.component.show-error-message-below-field]]
=== jmix.ui.component.show-error-message-below-field

控制是否直接在字段下方显示错误消息。默认值为 `true`，即错误消息在对应的组件下方显示。将此属性设置为 `false` 会隐藏这些内联消息；相反，当用户单击 *OK*（或类似的提交按钮）时，会显示包含所有错误的摘要通知。
