...
[[jmix.ui.exception-dialog-modal]]
=== jmix.ui.exception-dialog-modal

Это свойство приложения определяет, должен ли xref:exception-handlers.adoc#customizing-default-handler[ExceptionDialog] отображаться как модальное окно в Jmix-приложениях.

Если свойство включено, `ExceptionDialog` будет вести себя как модальное окно, блокируя взаимодействие с другими частями приложения, пока диалог не будет закрыт.

Если свойство выключено, `ExceptionDialog` будет вести себя как немодальное окно, позволяя пользователям продолжать взаимодействовать с другими частями приложения, пока диалог открыт.

Значение по умолчанию: `true`.

Смотрите также xref:flow-ui:exception-handlers.adoc[Обработчики исключений].
...
Значение по умолчанию: `jpg, png, jpeg, pdf`

[[jmix.ui.view.close-shortcut]]
=== jmix.ui.view.close-shortcut

Определяет xref:keyboard-shortcuts.adoc[клавишу быстрого доступа], которая используется для закрытия текущего экрана.

[[jmix.ui.view.save-shortcut]]
=== jmix.ui.view.save-shortcut

Определяет xref:keyboard-shortcuts.adoc[клавишу быстрого доступа], которая используется для сохранения изменений.
...
[[jmix.ui.ui-test-mode]]
=== jmix.ui.ui-test-mode

Включает генерацию специальных тестовых идентификаторов для использования с библиотекой тестирования Masquerade. Чтобы узнать больше, посетите раздел xref:testing:masquerade.adoc[].

[[components]]
== Компоненты

[[jmix.ui.component.checkbox-required-state-initialization-enabled]]
=== jmix.ui.component.checkbox-required-state-initialization-enabled

Это свойство управляет поведением компонентов xref:vc/components/checkbox.adoc[checkbox] и xref:vc/components/switch.adoc[switch], связанных с обязательным булевым атрибутом с валидацией `@NotNull`.

* Если выбрано `true`, то `checkbox`/`switch` должен быть отмечен, чтобы пройти валидацию.

* Если выбрано `false`, `checkbox`/`switch` может быть неотмеченным без возникновения ошибок валидации. Это позволяет ему представлять значения `true` или `false` по необходимости.
...
Определяет действия, которые необходимо добавить в компонент выбора для указанной сущности в механизмах стратегии генерации, таких как параметры xref:vc/components/genericFilter.adoc[genericFilter] или редактируемые ячейки xref:vc/components/dataGrid.adoc[dataGrid].
...
Определяет компонент, используемый для выбора указанной сущности в механизмах стратегии генерации, таких как параметры xref:vc/components/genericFilter.adoc[genericFilter] или редактируемые ячейки xref:vc/components/dataGrid.adoc[dataGrid].

Укажите полное имя класса (FQN) в качестве значения свойства. Например, чтобы использовать xref:vc/components/entityComboBox.adoc[entityComboBox] для выбора сущности `User`, задайте свойство следующим образом:
...
По умолчанию, общие механизмы используют компонент xref:vc/components/entityPicker.adoc[entityPicker].
...
Определяет xref:keyboard-shortcuts.adoc[клавишу быстрого доступа] для применения условий в компоненте xref:vc/components/genericFilter.adoc[genericFilter] в том случае если они не <<jmix.ui.component.filter-auto-apply, применяются автоматически>>.
...
Определяет xref:keyboard-shortcuts.adoc[клавишу быстрого доступа], которая используется для выполнения действия xref:actions/list-actions.adoc#list_add[list_add].

[[jmix.ui.component.grid-create-shortcut]]
=== jmix.ui.component.grid-create-shortcut

Определяет xref:keyboard-shortcuts.adoc[клавишу быстрого доступа], которая используется для выполнения действия xref:actions/list-actions.adoc#list_create[list_create].

[[jmix.ui.component.grid-edit-shortcut]]
=== jmix.ui.component.grid-edit-shortcut

Определяет xref:keyboard-shortcuts.adoc[клавишу быстрого доступа], которая используется для выполнения действия xref:actions/list-actions.adoc#list_edit[list_edit].
...
Определяет xref:keyboard-shortcuts.adoc[клавишу быстрого доступа], которая используется для выполнения действия xref:actions/list-actions.adoc#list_read[list_read].
...
Определяет xref:keyboard-shortcuts.adoc[клавишу быстрого доступа], которая используется для выполнения действия xref:actions/list-actions.adoc#list_remove[list_remove].
...
Определяет xref:keyboard-shortcuts.adoc[клавишу быстрого доступа], которая используется для очистки ввода компонента выбора.

[[jmix.ui.component.picker-lookup-shortcut]]
=== jmix.ui.component.picker-lookup-shortcut

Определяет xref:keyboard-shortcuts.adoc[клавишу быстрого доступа], которая используется для открытия экрана поиска для компонента выбора.

[[jmix.ui.component.picker-open-shortcut]]
=== jmix.ui.component.picker-open-shortcut

Определяет xref:keyboard-shortcuts.adoc[клавишу быстрого доступа], которая используется для открытия экрана деталей для выбранной сущности в компоненте выбора.
