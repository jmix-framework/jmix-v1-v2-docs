...
This property controls the behavior of a checkbox component bound to a mandatory boolean attribute with `@NotNull` validation.

* If `true`, the checkbox must be checked to pass validation.

* If `false`, the checkbox can remain unchecked without causing validation errors. This allows it to represent either a `true` or `false` value as needed.
