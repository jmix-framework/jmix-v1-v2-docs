...
* `addProperty()` - allows you to define the entity attributes (keys). The method accepts a name of the attribute and its type in the form of a xref:data-model:datatype.adoc[Datatype] or a Java class. In the latter case, the class should be either an entity class or a class supported by one of the datatypes. This method is used by the framework under the hood when you define properties of the key-value container declartively in XML as explained above.
...
[TIP]
====
Explore the following online examples to see key-value containers in action:

* https://demo.jmix.io/ui-samples/sample/data-grid-key-value[DataGrid with key-value container^]
* https://demo.jmix.io/ui-samples/sample/property-filter-key-value[PropertyFilter with key-value container^]
====
