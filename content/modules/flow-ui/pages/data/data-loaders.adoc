...
如果加载器没有在 XML 中配置 `readOnly="true"`，或者加载器通过编程的方式设置了 xref:flow-ui:data/data-context.adoc[DataContext]，则所有加载的实体都自动合并到数据上下文。
...
TIP: 访问 https://demo.jmix.io/ui-samples/sample/custom-filter[使用声明式条件自定义过滤^] 的在线示例了解查询条件的实操用法。
