...
<div class="jmix-ui-live-demo-container">
    <a href="https://demo.jmix.io/ui-samples/sample/fragments" class="live-demo-btn" target="_blank">在线示例</a>
</div>
++++
...
* Studio 提供了 xref:studio:view-wizard.adoc#creating-blank-fragment[Blank fragment] 模板可以生成空的 fragment，然后在 Jmix 工具窗口展示，与视图一样，也可以使用 xref:studio:view-designer.adoc[可视化设计器]。

Fragments 目前不支持 xref:flow-ui:facets.adoc[facets]。

TIP: 使用 Fragments 可以在某些列表 UI 组件内部对条目进行渲染，例如 xref:vc/components/virtualList.adoc[virtualList] 或 xref:vc/components/dataGrid.adoc[dataGrid] 的列。通过 XML 内部的 xref:flow-ui:vc/miscellaneous/renderers.adoc#fragment-renderer[fragmentRenderer] 元素实现。 `fragmentRenderer` 元素设置一个继承自 `io.jmix.flowui.fragmentrenderer.FragmentRenderer` 的 Java 类。这个类负责使用提供的数据条目在 fragment 模板内进行渲染。
...
* `actions` - 可选元素，设置 fragment 内的操作（与视图的 `actions` 元素类似）。如果操作有 xref:keyboard-shortcuts.adoc[快捷键]，则将与片段的根组件绑定。也就是说，只有当焦点位于片段布局内时，才能触发快捷键。
