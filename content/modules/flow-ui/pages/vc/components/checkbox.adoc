...
Компонент `checkbox` поддерживает валидацию, чтобы обеспечить соответствие его состоянию `required`. Чекбокс может быть обозначен как обязательный либо через его собственный атрибут `required`, либо потому что он связан с обязательным атрибутом сущности. Если чекбокс считается обязательным любым из этих способов, валидация успешно пройдет только в том случае, если чекбокс отмечен, то есть если его значение равно `true`. Если пользователь попытается продолжить, не отметив обязательный чекбокс, валидация не пройдет, и окно останется открытым.
...
|`false`
|[[required]]required
|Устанавливает обязательное состояние для `checkbox`. Смотрите <<required-state,Состояние Required>>.
|`false`
|===
