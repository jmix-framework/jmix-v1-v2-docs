...
Sets the text that should be shown if the value is set from the data container. When a user uploads the file, the field shows the file name. However, when the value is saved to the database, the field loses information about the file name. The `fileName` attribute is intended to set custom text when there is no information about the file name. If it is not defined, the default value will be shown:
