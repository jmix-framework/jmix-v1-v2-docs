...
`multiValuePicker` 可以处理任意类型值的列表。由一个文本框和一组由操作定义的按钮组成。
[cols="1h,6", grid=none]
|===
|XML Element
|`multiValuePicker`
...
`multiValuePicker` 可以通过操作编辑列表值，而不是在文本字段内输入进行编辑。
...
NOTE: *Select Value* 视图可以自定义。该自定义视图需要实现 `MultiValueSelectView` 接口，然后通过操作的 `viewClass` 或 `viewId` 属性设置该视图。
