...
The *Remember me* checkbox allows the user to maintain their login state for a long period of time, longer than the lifespan of an HTTP session. This state remains even after the server restarts.

When the user selects the *Remember me* checkbox during login, the system stores a persistent token in a browser cookie. On subsequent visits, the user is automatically authenticated without entering credentials again.

To handle the `Remember me` state in your login logic, use the <<RememberMeChangedEvent,RememberMeChangedEvent>>.

See https://docs.spring.io/spring-security/reference/servlet/authentication/rememberme.html[Spring Security Remember-Me documentation^] for more information.
...
Sets the visibility of the *Remember me* checkbox. When `true`, users can choose to stay logged in across browser sessions.
...
See https://vaadin.com/docs/v24/components/login[Vaadin Docs^] for more information.