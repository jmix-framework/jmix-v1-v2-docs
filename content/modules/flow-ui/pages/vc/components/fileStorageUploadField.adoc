...
++++
<div class="jmix-ui-live-demo-container">
    <a href="https://vaadin.com/docs/v24/components/upload/#drag-drop" class="vaadin-docs-btn" target="_blank">Vaadin</a>
</div>
++++
...
++++
<div class="jmix-ui-live-demo-container">
    <a href="https://vaadin.com/docs/v24/components/upload/#file-size" class="vaadin-docs-btn" target="_blank">Vaadin</a>
</div>
++++

Specifies the maximum file size in bytes allowed for upload. Note that this is a client-side constraint, which is checked before sending the request to the server.

[NOTE]
====
If `MultipartProperties` is available, the default value is taken from `MultipartProperties#getMaxFileSize()` (which defaults to `1MB`).

To increase the maximum file size for all fields globally, use the following Spring Boot application properties:

* https://docs.spring.io/spring-boot/appendix/application-properties/index.html#application-properties.web.spring.servlet.multipart.max-file-size[spring.servlet.multipart.max-file-size^] – sets the maximum allowed size of an uploaded file (for example, `10MB`)

* https://docs.spring.io/spring-boot/appendix/application-properties/index.html#application-properties.web.spring.servlet.multipart.max-request-size[spring.servlet.multipart.max-request-size^] – sets the maximum allowed size of the entire multipart request (for example, `10MB`)

For more options, refer to the Spring Boot documentation.

CAUTION: These properties apply to JAR deployment. For WAR deployment, you may need to configure the file size limits in your application server settings.
====
...
`FailedEvent` occurs when a file upload is received by the server but the reception process is interrupted before completion. Common causes include network failures, server-side processing errors, or explicit cancellation.
...
This event indicates that a user-selected file has been rejected because it fails to meet one or more constraints configured on the upload component, such as:

* File size limits (<<maxFileSize,maxFileSize>>)
* Accepted file types (<<acceptedFileTypes,acceptedFileTypes>>).

Use this event to provide user feedback when upload constraints are violated.
...
This event signals that a file upload process has completed, regardless of its outcome. It is fired for both successful uploads and failed attempts.

For more granular control over the upload result, consider using these specific sub-events instead:

* <<FileUploadSucceededEvent,FileUploadSucceededEvent>> - indicates a successful file upload.

* <<FileUploadFailedEvent,FileUploadFailedEvent>> - indicates a failed or interrupted upload.
...
This event enables real-time tracking of an ongoing file upload. It provides continuous feedback on the upload progress, allowing you to update UI elements.
...
This event signals the beginning of a file upload process, immediately after a user selects a file and the transfer to the server is initiated.
...
This event confirms the successful completion of a file upload, indicating that the file has been fully received and processed by the server without errors.

Here is an example of using `fileStorageUploadField` for `.xls` and `.xlsx` files, and then processing those files in `FileUploadSucceededEvent`.
