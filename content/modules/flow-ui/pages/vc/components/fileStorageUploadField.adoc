...
NOTE: 上传的文件会立即保存在默认的 `FileStorage` 中。如需以编程的方式控制文件的存储，可以使用 <<fileStoragePutMode,fileStoragePutMode>> 属性。
...
下面是使用 `fileStorageUploadField` 上传 `.xls` 和 `.xlsx` 文件的示例，文件在 `FileUploadSucceededEvent` 事件中处理。
