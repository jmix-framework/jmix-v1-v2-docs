...
Переключатель (`switch`) позволяет пользователям переключаться между двумя состояниями: `true` (включено) и `false` (выключено). Это делает компонент идеальным для: включения/отключения настроек, управления предпочтениями, работы с бинарными опциями.
