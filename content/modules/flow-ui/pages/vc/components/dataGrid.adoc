...
通常，当用户触发确认操作时，可视化组件对实体实例的修改会通过 xref:flow-ui:data/data-context.adoc[] 保存至数据库。

如果需要在用户离开当前单元格时自动保存行内编辑的修改，可以为数据容器创建一个 `ItemPropertyChangeEvent` 事件的处理器并保存 `DataContext`：
...
如果 `dataGrid` 中的实体没有合并到 `DataContext` 中（例如，`loader` 是 `readOnly="true"`），则可以通过 `DataManager` 保存实体：
