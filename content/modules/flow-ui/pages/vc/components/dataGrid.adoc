...
По умолчанию компонент работает в режиме одиночного выбора. Когда вы задаёте <<selectionMode, selectionMode>> значение `MULTI`, появляется столбец с чекбоксами, позволяющий выбрать любое количество строк. Чтобы выбрать все строки, щёлкните по чекбоксу в заголовке таблицы.
...
----
<1> Сделайте нужные столбцы <<editable, редактируемыми>>.
...
<2> Сделать соответствующие столбцы <<editable,редактируемыми>>.
<3> Кнопка для начала редактирования.
<4> Кнопка для подтверждения изменений.
<5> Кнопка для отмены изменений.
====
// TIP: Inline editing can be configured programmatically as described in the https://vaadin.com/docs/v24/components/grid#inline-editing-java-only[Vaadin documentation^]
...
CAUTION: Для поддержки механизмов фреймворка, таких как контейнеры данных, источники значений и т.д., компонент редактирования столбца должен добавляться с использованием методов `DataGridEditor` (например, `DataGridEditor#setColumnEditorComponent()`), а не через прямое API столбца `Column#setEditorComponent()`.
...
Данные в `dataGrid` можно фильтровать с помощью встроенных в заголовки столбцов xref:flow-ui:vc/components/propertyFilter.adoc[фильтров свойств].

Вы можете указать, для каких столбцов должен быть доступен фильтр, используя XML-атрибут `filterable`. В фильтруемых столбцах в заголовках отображается иконка фильтра (image:visual-components/components/funnel.svg[]). При клике на эту иконку появляется диалог с условием фильтрации по свойству. Если условие задано, иконка в соответствующем столбце подсвечивается.
...
Фильтры свойств в заголовках столбцов работают так же, как отдельные фильтры по свойствам и как xref:flow-ui:vc/components/genericFilter.adoc[] - они добавляют условия к xref:flow-ui:data/data-loaders.adoc[загрузчику данных]. В стандартном режиме эти условия преобразуются в JPQL-запрос и фильтруют данные на уровне базы данных.
...
++++
<div class="jmix-ui-live-demo-container">
    <a href="https://vaadin.com/docs/v24/components/grid/flow/#using-renderers-in-columns" class="vaadin-docs-btn" target="_blank">Vaadin</a>
</div>
++++
...
[TIP]
====
Для добавления рендерера в Jmix Studio выберите элемент `column` XML-дескрипторе экрана или в панели структуры *Jmix UI*, затем нажмите кнопку xref:studio:screen-designer.adoc#component-inspector[Add-><Some>Renderer] в панели инспектора *Jmix UI*.
====
...
[TIP]
====
Элемент <<column,column>> может быть определён без атрибута <<property,property>>, то есть без прямой привязки к атрибуту сущности. В этом случае элемент `column` должен иметь атрибут <<key,key>> с уникальным значением.
====
+
Примеры использования можно посмотреть <<text-renderer,здесь>> и <<component-renderer,здесь>>.

[[fragment-renderer]]
* *Используя FragmentRenderer*
+
Колонки можно отображать с помощью вложенного элемента `fragmentRenderer`. Подробнее смотрите в разделе xref:flow-ui:vc/miscellaneous/renderers.adoc#fragment-renderer[Рендерер фрагментов].
...
----
<1> Добавляет новый столбец, который отображает компонент `image`.
<2> Экземпляр компонента `Image` создается с помощью фабрики `UiComponents`.
<3> Создает `DownloadHandler`, который открывает входной поток (input stream) для файла, на который ссылается атрибут `picture` сущности `User`.
<4> Устанавливает `DownloadHandler` в качестве источника (source) компонента `Image`.
...
|Если значение `true`, активирует агрегацию столбцов. Смотрите раздел <<aggregating, Агрегация данных>>.
|`false`
...
|`false`
|[[editable]]editable
|Если значение равно `true`, столбец можно редактировать. Подробнее смотрите раздел <<inline-editing, Встроенное редактирование>>.
|`false`
|[[filterable]]filterable
|Если значение равно `true`, для этого столбца включается фильтрация. Подробнее смотрите раздел <<column-filtering,Фильтрация по заголовкам столбцов>>.
|`false`
|[[flexGrow]]flexGrow
|Задаёт коэффициент расширения (flex grow) для данного столбца. При значении `0` ширина столбца фиксирована.
|`0`
|[[footer]]footer
|Задаёт текст в нижней части столбца (футере). Значением атрибута может быть либо сам текст, либо ключ из xref:localization:message-bundles.adoc[пакета сообщений]. В случае использования ключа значение должно начинаться с префикса `msg://`.
| –
|[[frozen]]frozen
...
|`false`
|[[header]]header
|Задаёт текст заголовка столбца. Значением атрибута может быть либо сам текст, либо ключ из xref:localization:message-bundles.adoc[пакета сообщений]. В случае использования ключа значение должно начинаться с префикса `msg://`.
| –
|[[key]]key
...
Смотрите {vaadin-docs}/components/grid[документацию Vaadin^] для получения дополнительной информации.