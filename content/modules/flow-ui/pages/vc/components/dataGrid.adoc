...
Typically, changes to entity instances made by visual components are saved to the database by xref:flow-ui:data/data-context.adoc[] when a confirmation action is triggered by the user.

To automatically commit inline edits when the user leaves the current cell, create a handler for the data container's `ItemPropertyChangeEvent` and save the `DataContext`:

[source,java,indent=0]
----
include::example$/onboarding/src/main/java/com/company/onboarding/view/component/datagrid/DataGridFeaturesView.java[tags=auto-save-data-context]
----

If the entities in the `dataGrid` are not merged into the `DataContext` (for example if the `loader` has the `readOnly="true"` attribute), save the changed entity using `DataManager`:
...
TIP: Both the header and footer can contain rich content and components. See https://demo.jmix.ru/ui-samples/sample/data-grid-header-footer[live demo^].
...
|Generates parts of CSS class names for this column based on given conditions. This allows for customizing cell appearance based on the data displayed. See https://demo.jmix.ru/ui-samples/sample/data-grid-part-name-generator[live demo^].
|[[renderer]]renderer
