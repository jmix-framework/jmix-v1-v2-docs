...
* 任何带 `@ThemeUtilityClasses` 注解的自定义类。可以在自己的扩展组件或应用程序源代码中提供共享类名。

Jmix Studio 提供对输入 `classNames` 属性的增强支持，在 UI 组件属性中输入时，会显示 xref:studio:view-designer.adoc#classNames-editor[ClassNames 向导]。
...
定义触发组件点击事件的 xref:keyboard-shortcuts.adoc[键盘快捷键]。组件必须有处理点击事件的相应方法，否则快捷键不会有任何效果。
...
设置组件内容的 xref:data-model:datatype.adoc[数据类型]。支持：
...
定义组件获得焦点的 xref:keyboard-shortcuts.adoc[键盘快捷键]。
...
请参考布局规则的 xref:vc/layout-rules.adoc#component-sizes[用例]。
