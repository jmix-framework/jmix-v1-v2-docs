...
Defines the default page size for xref:ui:vcl/components/pagination.adoc[] and xref:ui:vcl/components/simple-pagination.adoc[] components.

Default value: `50`

See also <<jmix.ui.entity-page-size,jmix.ui.entity-max-page-size>>.
...
Defines the default page size for xref:ui:vcl/components/pagination.adoc[] and xref:ui:vcl/components/simple-pagination.adoc[] components for a particular entity.

The following example configures the default page size for two entities. For all other entities the value from <<jmix.ui.default-page-size,jmix.ui.default-page-size>> is used.
...
Determines whether an xref:ui:dialogs.adoc#exception-dialog[Exception dialog] should be opened as modal by default.
...
The icons from the registered icon sets are displayed in the *ICON SET* tab of the xref:studio:screen-designer.adoc#icon-editor[Choose icon] dialog.
...
Set to `true` when deploying the application to production. It will set the `productionMode` Vaadin servlet parameter.
...
For the xref:files:downloading-files.adoc#downloader[Downloader] bean, defines the list of file extensions to be shown in the web browser tab instead of downloading them automatically.
...
Defines the keyboard shortcut that is used to apply current filtering conditions in a xref:vcl/components/filter.adoc[] component in case they are not <<jmix.ui.component.filter-auto-apply, automatically applied>>.
...
This property can be overridden for a particular xref:vcl/components/filter.adoc[] component using its `applyShortcut` XML attribute.
