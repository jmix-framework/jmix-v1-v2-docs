...
TIP: See the xref:studio:composite-projects.adoc[] section for how to develop multiple add-ons and applications in a single IDE project.
...
This approach is widely used in the framework for aggregating configuration defined in add-ons, like xref:flow-ui:menu-config.adoc[UI menu], shared xref:data-access:fetching.adoc#fetch-plan[fetch plans], and others.
