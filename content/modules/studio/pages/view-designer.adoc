...
[[classNames-editor]]
* The xref:flow-ui:vc/common-attributes.adoc#classNames[classNames] attribute supports autocompletion in the value input field. Additionally, clicking the image:icons/edit-new.svg[Edit] button opens the *ClassNames Wizard* dialog, which allows you to select css class names visually.
+
image::class-names-wizard.png[align="center",width="912"]
+
Available css class names are obtained from the following sources:

** `com.vaadin.flow.theme.lumo.LumoUtility` class and its nested classes.
** `io.jmix.flowui.themes.JmixLumoUtility` class.
** Any custom class annotated with `@ThemeUtilityClasses`.
// +
