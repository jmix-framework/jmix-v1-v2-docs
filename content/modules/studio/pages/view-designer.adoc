...
The properties of the selected component are grouped into categories for easier navigation. To find a specific property, either browse through the relevant category or use the search field at the top of the component inspector. You can reorder the categories using the image:icons/editorConfig.svg[Categories Order] button. Buttons image:icons/collapseAll.svg[Collapse All] and image:icons/expandAll.svg[Expand All] control the display of the categories, allowing you to collapse or expand them.
...
Now, let's go back to the list of properties and take a closer look.

The first column lists the names of the attributes for the selected component. The second column displays the property values as follows:
