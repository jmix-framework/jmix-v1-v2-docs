...
image::component-properties-panel.png[align="center",width="337"]

Свойства компонента организованы в категории для удобного просмотра. Чтобы найти определенное свойство, просмотрите соответствующую категорию или используйте поле поиска в верхней части инспектора компонентов. Вы можете изменить порядок категорий с помощью кнопки image:icons/editorConfig.svg[Categories Order]. Кнопки image:icons/collapseAll.svg[Collapse All] и image:icons/expandAll.svg[Expand All] управляют отображением категорий, позволяя вам сворачивать или разворачивать их.
...
image::component-inspector-add-button.png[align="center", width="357"]

[[add-picker-action]]
* Чтобы добавить xref:flow-ui:actions/entity-picker-actions.adoc[действие компонента entityPicker] или xref:flow-ui:actions/value-picker-actions.adoc[действие компонента valuePicker], выберите соответствующий компонент в XML-дескрипторе экрана или на панели структуры *Jmix UI* и нажмите *Add → Action* в панели инспектора *Jmix UI*. Появится диалоговое окно *Add Action*:
+
...
[[add-action]]
* Чтобы добавить xref:flow-ui:actions/list-actions.adoc[действие с коллекцией], выберите компонент xref:flow-ui:vc/components/dataGrid.adoc[dataGrid] или xref:flow-ui:vc/components/treeDataGrid.adoc[treeDataGrid] в XML-дескрипторе экрана или на панели структуры *Jmix UI* и нажмите *Add → Action* в панели инспектора *Jmix UI*. Появится диалоговое окно *Add Action*:
+
...
[[add-column]]
* Чтобы добавить `column`, выберите компонент `dataGrid` или `treeDataGrid` в XML-дескрипторе экрана или на панели структуры *Jmix UI* и нажмите *Add → Columns → Column* на панели инспектора *Jmix UI*. Появится диалоговое окно *Add Column*:
+
...
А теперь вернемся к списку свойств и рассмотрим его более детально.

В первом столбце указаны названия атрибутов выбранного компонента. Во втором столбце отображаются значения свойств следующим образом:
...
[[classNames-editor]]
* Атрибут xref:flow-ui:vc/common-attributes.adoc#classNames[classNames] поддерживает автозавершение в поле ввода значения. Кроме того, при нажатии кнопки image:icons/edit-new.svg[Edit] открывается диалоговое окно *ClassNames Wizard*, которое позволяет визуально выбирать имена CSS-классов.
+
image::class-names-wizard.png[align="center",width="912"]
+
Доступные имена CSS-классов берутся из следующих источников:

** Класс `com.vaadin.flow.theme.lumo.LumoUtility` и его вложенные классы.
** Класс `io.jmix.flowui.themes.JmixLumoUtility`.
** Любые пользовательские классы, аннотированные `@ThemeUtilityClasses`.
// +
...
// ** The *ICON SET* tab contains icons from the xref:ui:icons/custom-icons.adoc[custom icon sets].
