...
* *New -> Liquibase Changelog* - creates a new empty xref:data-model:db-migration.adoc#changelogs[changelog] file.
...
* *Generate Liquibase Changelog* - generates changelog files for the selected data store by comparing the data model with the database schema. Applicable for the main data store and additional data stores with *Update Only* and *Create and Update* schema management types.
+
TIP: Refer to the xref:data-model:db-migration.adoc#ignoring-existing-tables[Database Schema Migration -> Ignoring Existing Tables] section for instructions on how to exclude existing tables from handling by Studio.
+
