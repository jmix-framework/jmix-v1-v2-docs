...
Есть три коммерческих плана подписки: *Sprint*, *Enterprise*  и *BPM*. Сравнение бесплатного и коммерческих планов можно найти на странице https://www.jmix.ru/subscription-plans-and-prices/[Тарифные планы и стоимость подписки^] веб-сайта Jmix.

[[sprint-subscription]]
=== Sprint подписка

Подписка Sprint открывает следующие премиальные функции Studio:

* xref:studio:entity-designer.adoc[Дизайнеры сущностей и перечислений]
* xref:studio:data-stores.adoc#data-store-actions[Генерация и агрегация файлов Liquibase changelog]
...
* xref:studio:data-repository-wizard.adoc#creating-repository-methods[Дизайнер методов репозиториев данных]
* xref:openapi-integration-guide:index.adoc[Генерация клиентов по OpenAPI схеме]
...
При первом входе в Studio вы получите пробную Sprint подписку. Она действует 28 дней и позволяет оценить премиум-функции в проектах любого размера.
...
=== Enterprise и BPM подписки

Подписки *Enterprise* и *BPM* включают в себя все функции плана Sprint, а кроме того, предоставляют доступ к коммерческим дополнениям.

* Подписка Enterprise включает xref:business-calendar:index.adoc[], xref:kanban:index.adoc[], xref:maps:index.adoc[], xref:notifications:index.adoc[], xref:tabbed-mode:index.adoc[], xref:uiconstraints:index.adoc[] и xref:webdav:index.adoc[].

* Подписка BPM включает все дополнения выше плюс дополнение xref:bpm:index.adoc[] и xref:studio:bpmn-designer.adoc[дизайнер BPMN] в Studio.
...
Вы можете опробовать коммерческие дополнения, запросив пробную Enterprise+BPM подписку, как описано в разделе xref:ROOT:account-management.adoc#enterprise-trial[Управление учетной записью, выставлением счетов и подпиской].
