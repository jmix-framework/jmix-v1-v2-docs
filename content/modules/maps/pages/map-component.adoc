...
* `projection` относится к системе координат, в которой отображается карта. Распространенные проекции включают `EPSG:3857` и `EPSG:4326`. Проекция по умолчанию - https://epsg.io/3857[EPSG:3857^]. Вы можете установить пользовательскую проекцию в специальном внутреннем элементе `projection`. Подробнее смотрите https://openlayers.org/en/v8.2.0/apidoc/module-ol_proj_Projection-Projection.html[Projection^].
