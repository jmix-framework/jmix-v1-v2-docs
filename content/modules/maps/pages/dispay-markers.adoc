...
Double-click the `User` entity in the *Jmix* tool window and choose its last attribute (to append the new attribute at the end):
...
The dialog shows all views that display the `User` entity. Select the `User.detail` view:
...
Add the logic for opening `LocationLookupView` to the `ActionPerformedEvent` handler method:
...
First, add a field where the current location selected on the map will be shown. Navigate to the actions panel, click *Add Component*, locate `entityPicker`, and double-click it. Configure the component properties as follows:
...
To show the list of locations, we'll use the `listBox` component. First, add a data container that will supply a collection of `Location` entities for the list. To do this, click *Add Component* in the actions panel, go to the `Data components` section, and double-click the `Collection` item. In the *Collection Properties Editor* window, select `Location` in the *Entity* field and click *OK*:
