...
* Migrates imports of `NotInstantiatedList` and `NotInstantiatedSet` classes. See details <<core-modules-refactoring,below>>.
* If the project includes the REST API add-on, sets the xref:rest:app-properties.adoc#jmix.rest.inline-fetch-plan-enabled[jmix.rest.inline-fetch-plan-enabled] to `false`. See details <<fetch-plans-in-rest-api-and-rest-datastore,below>>.
* If the project includes the Authorization Server add-on, sets the xref:authorization-server:configuration.adoc#jmix.authserver.use-in-memory-authorization-service[jmix.authserver.use-in-memory-authorization-service] property to `true`. See details <<authorization-server-token-storage,below>>.
...
The new xref:tabbed-mode:index.adoc[] add-on allows you to open application views in separate tabs inside the main view.

The add-on transparently replaces xref:flow-ui:views/opening-views.adoc#navigation[navigation] by opening views in the main view tabs. You can also use the specific `ViewBuilders` bean to open views in the current main view tab, in a new tab, or in a dialog window.
...
You can disable inline fetch plans for the generic REST in your application using the xref:rest:app-properties.adoc#jmix.rest.inline-fetch-plan-enabled[jmix.rest.inline-fetch-plan-enabled] application property. When you use Studio to upgrade an existing project to Jmix 2.5, it sets this property to `false` automatically to prevent accidental data leaks.
...
The *Jmix Sessions* subsystem provides support for user sessions maintained across REST requests with the same token. See xref:security:authentication.adoc#sessions-in-rest-api[Sessions in REST API] for more information.
...
You can use `Copier` to isolate entities from UI when sending them to custom services from views. `DataContext` uses this interface to make entity copies when saving them to `DataManager`. See more information in the xref:flow-ui:data/entities-in-ui.adoc[] section.
...
If you want to revert to the previous in-memory token storage, set the xref:authorization-server:configuration.adoc#jmix.authserver.use-in-memory-authorization-service[jmix.authserver.use-in-memory-authorization-service] application property to `true`.  When you use Studio to upgrade an existing project to Jmix 2.5, it sets this property to `true` automatically to facilitate the process.
...
Due to internal refactoring (https://github.com/jmix-framework/jmix/issues/4152[#4152]), constructor signatures of the following beans have been changed:
...
** https://github.com/jmix-framework/jmix/issues?q=is%3Aclosed+milestone%3A2.5.2[2.5.2^]
...
** https://youtrack.jmix.io/issues/JST?q=Fixed%20in%20builds:%202.5.2[2.5.2^]
