...
Jmix Studio now supports using `.env` files to externalize configuration. You can store environment-specific variables separately from `application.properties` and use them with placeholders like `++${DB_URL}++`.
...
image::jmix:flow-ui:visual-components/layouts/side-panel-layout-editor.png[width=1150]

The side panel can open from different sides and can either overlay the main content or push it aside. See xref:jmix:flow-ui:vc/layouts/sidePanelLayout.adoc[] for details and usage examples.
...
image::jmix:flow-ui:dialogs/side-dialog.png[width=1155]

Use it for temporary interactions that should not be embedded directly into the view layout. See xref:jmix:flow-ui:dialogs.adoc#side-dialog[SideDialog] for details and usage example.
...
. To avoid Liquibase checksum error for executed changelogs, run your application once with `main.liquibase.clear-checksums=true` property. For example, if your main data store uses MariaDB, execute the following command:
+
[source,shell]
----
