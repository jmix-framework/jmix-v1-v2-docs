...
Подсистема *Jmix Sessions* предоставляет поддержку сессий, поддерживаемых для REST-запросов с одним и тем же токеном. Подробнее см. xref:security:authentication.adoc#sessions-in-rest-api[Сессии в REST API].
...
Вы можете использовать `Copier` для изоляции сущностей от UI при отправке их в собственные сервисы из экранов. `DataContext` использует этот интерфейс для создания копий сущностей при их сохранении в `DataManager`. Подробнее см. раздел xref:flow-ui:data/entities-in-ui.adoc[].
