...
* Coarse-grained roles define all permissions required for a specific job, like "Salesperson". Such a role can define all permissions itself or inherit them from base roles, so a course-grained role can be made as an aggregation of fine-grained ones.
