...
* loaded as a root, and
* also appear in nested collections/associations

then define both JPQL and <<predicate-policy,predicate>> row-level policies for that entity. JPQL for efficient database-level filtering of root loads, predicate for nested loads.
====
...
then define both <<jpql-policy, JPQL>> and predicate row-level policies for that entity. JPQL for efficient database-level filtering of root loads, predicate for nested loads.
====
+
