...
To completely ignore Spring Security rules for static public resources, create a configuration class extending `FlowuiVaadinWebSecurity` and register your public URL patterns with the `ignoring()` method of `WebSecurity`:

[source,java,indent=0]
----
include::example$/security-ex1/src/main/java/com/company/demo/security/DemoSecurityConfiguration.java[]
----

The example configuration above gives public access to all files located inside the `src/main/resources/public/images` directory of your project (see how to serve static content in the https://docs.spring.io/spring-boot/docs/current/reference/html/web.html#web.servlet.spring-mvc.static-content[Spring Boot documentation^]).
...
The configuration above grants access to all requests that match the `/public-api/**` pattern.
