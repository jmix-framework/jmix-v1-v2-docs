...
Значения параметров должны передаваться в формате, определенном для соответствующего xref:data-model:datatype.adoc[datatype].
...
Запрос должен иметь следующую структуру:
...
[NOTE]
====
Вместо POJO рекомендуется использовать xref:data-model:entities.adoc#dto[DTO-сущности]. Это предоставляет такие преимущества, как правильная генерация схемы OpenAPI и поддержка Bean Validation для валидации параметров запроса.
====
