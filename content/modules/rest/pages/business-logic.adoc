...
* *基于注解的方式*：Spring bean 中必须使用 `@RestService` 注解创建。
* *基于 XML 配置的方式*：Spring bean 必须满足下列条件：
...
当使用结构化请求对象作为方法参数时，JSON 的请求体必须将参数名设置为顶级的键值。
...
[NOTE]
====
除了使用 POJO 作为参数对象外，我们建议使用 xref:data-model:entities.adoc#dto[DTO 实体]。这种方式提供了多个优势，例如能正确生成 OpenAPI 架构，以及在请求参数验证时更好地支持 bean 验证。
====
...
如需为自定义控制器提供与 Jmix REST API 一样的 OAuth2 保护机制，在 xref:authorization-server:protecting-resources.adoc#protecting-rest-api-endpoints[jmix.resource-server.authenticated-url-patterns] 应用程序属性中注册控制器的 URL pattern：
