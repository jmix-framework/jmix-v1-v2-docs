...
NOTE: To upload a file, the user must have a role with the `rest.fileUpload.enabled` xref:security:resource-roles.adoc#specific-policy[specific policy].
...
The list of file extensions that can be downloaded with `Content-Disposition: inline` is set in the xref:app-properties.adoc#jmix.rest.inline-enabled-file-extensions[jmix.rest.inline-enabled-file-extensions] application property.

NOTE: To download a file, the user must have a role with the `rest.fileDownload.enabled` xref:security:resource-roles.adoc#specific-policy[specific policy].
