...
fetchPlan:: 实体 xref:data-access:fetching.adoc[fetch plan] 的名称或 URL 加密的 JSON 对象 `（String）`。
...
为此，实体加载 API 支持了 xref:data-access:fetching.adoc[Fetch Plan]。使用 Fetch plan 可以配置一次从数据库需要加载的属性树，并传递给客户端，提高了性能并减少了开销。

[[named-fetch-plans]]
=== 命名 Fetch Plans

命名 fetch plan 是指在应用程序共享存储库中注册的 fetch plan（参阅 xref:data-access:fetching.adoc#creating-fetch-plans[创建 fetch plan] 部分）。可以通过指定 fetch plan 的名称在 REST 查询中使用。
...
下面示例中 ID 为 `21021f78-edac-224b-e6f8-6e71e02a0f0d` 的 Order 通过 fetch plan `order-with-details` 进行加载，此时会加载额外的 `customer` 和 `lines` 数据：
...
当使用 POST 请求加载数据时，例如在 <<load-list-search-filter,搜索>> 端点中，可以在请求体的 JSON 的
`fetchPlan` 属性中指定 fetch plan 的名称：
...
内联 fetch plan 可以通过 xref:app-properties.adoc#jmix.rest.inline-fetch-plan-enabled[jmix.rest.inline-fetch-plan-enabled] 应用程序属性禁用。
