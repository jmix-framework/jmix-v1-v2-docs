...
<2> Create an `io.jmix.email.EmailInfo` object using `io.jmix.email.EmailInfoBuilder`.
<3> Send the email synchronously.
+
...
The `io.jmix.email.EmailAttachment` object is a wrapper that holds the attachment as a byte array, a file name, and, if necessary, an attachment identifier which is unique for this message.
