...
If `true` enables using OAuth 2.0 token-based authorization to connect to email server.

[[jmix.email.oauth2.provider]]
=== jmix.email.oauth2.provider

Specifies email OAuth provider.

[[jmix.email.oauth2.client-id]]
=== jmix.email.oauth2.client-id

Specifies OAuth client id.

[[jmix.email.oauth2.secret]]
=== jmix.email.oauth2.secret

Specifies OAuth client secret.

[[jmix.email.oauth2.tenant-id]]
=== jmix.email.oauth2.tenant-id

Specifies OAuth tenant id.

[[jmix.email.oauth2.refresh-token]]
=== jmix.email.oauth2.refresh-token

Specifies OAuth refresh token.
...
----
<1> Start building a job definition using the `org.quartz.JobBuilder` class.
<2> Create an `org.quartz.JobDetail` instance.
<3> Start building a trigger definition using the `org.quartz.TriggerBuilder` class.
<4> Link this trigger to the previously defined job.
<5> Create an `org.quartz.Trigger` instance.
...
. In the running application, open the *Quartz -> Quartz jobs* view, and configure a job for the `MyCustomEmailCleaningJob` class.
...
. Ensure the following line is present in the `dependencies` section of your `build.gradle` file:
+
